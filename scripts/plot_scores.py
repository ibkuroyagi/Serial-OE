...
for i, machine in enumerate(machines):
    yerr_se = anomaly_score_list[i].std(1) / np.sqrt(len(seed_list))
    ddcsad_se = ddcsad_score_list[i].std(1) / np.sqrt(4)
    x = np.arange(len(n_anomaly_list))
    y_mean = anomaly_score_list[i].mean(1)
    ddcsad_mean = ddcsad_score_list[i].mean(1)
    if not use_one_fig:
        fig, ax = plt.subplots()
    ax.plot(x, y_mean, marker="o", label=f"Proposed method {machine}")
    ...
    if not use_one_fig:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"exp/fig/{no}_w_ddcsad_{machine}.png")

yerr_se = anomaly_score_list.mean(0).std(1) / np.sqrt(len(seed_list))
ddcsad_se = ddcsad_score_list.mean(0).std(1) / np.sqrt(4)
x = np.arange(len(n_anomaly_list))
y_mean = anomaly_score_list.mean(0).mean(1)
ddcsad_mean = ddcsad_score_list.mean(0).mean(1)
if not use_one_fig:
    fig, ax = plt.subplots()
ax.plot(x, y_mean, marker="o", label="Proposed method average")
